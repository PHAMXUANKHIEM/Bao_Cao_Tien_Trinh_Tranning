...
# default.rgw.buckets.index         0 B      11     ...
```

**Tại sao?** Bucket index listing data được lưu **entirely as OMAP entries** trong RocksDB database của OSDs, **không phải** trong RADOS object payload.

Điều này confirm tại sao leveraging **fast flash media (SSDs, NVMe)** cho OSD DB partition là essential.
...
### Giải pháp: DBR (Tái phân đoạn Bucket động)
...
- **Không hiệu quả**: các thao tác liệt kê phải kiểm tra mọi phân đoạn.
...
### Thực hành tốt nhất về phân đoạn trước

Nếu biết bucket sẽ khổng lồ ngay từ khi tạo:
- **Thực hành tốt nhất**: phân đoạn trước bucket khi tạo.
- Đặt số lượng phân đoạn ban đầu phù hợp.
- Tránh sự kiện tái phân đoạn động đầu tiên → hiệu suất tối ưu từ đối tượng đầu tiên.

---
## 8. Future Evolution: In-Order Sharding

### Current Limitation: Hash-based Sharding

- RGW's hashed sharding optimized cho write distribution.
- **Challenge**: listing objects theo alphabetical order khó.
- Để fulfill paginated list request: RGW must perform "**scatter-gather**" operation → query every single shard, sort combined results.
- Bottleneck cho buckets với very large number of shards.

### Solution: In-Order Sharding (Ordered Bucket Listing)

**Under development**: significant new feature sắp tới.

**Change**:
- Sharding logic place objects vào shards dựa trên **lexicographical name** (thay vì hash).

**Impact**:
- Thay vì query tất cả shards, request list objects được directed đến **specific shard(s)** containing requested alphabetical range.
- **Paginated listing operations** → dramatically faster, more efficient.
- Particularly beneficial cho workloads relying heavily on browsing/iterating through object keys.

### Combined Benefits

Khi combine:
- Automated scaling của **Dynamic Bucket Resharding** +
- Listing efficiency của **in-order sharding**

→ Ceph RGW trên path to provide **virtually limitless, performant scalability** trong single bucket.

→ Cater đến most demanding data lake và AI/ML use cases của future.

---

**Phần 2** tiếp tục khám phá:
- Bố cục metadata control plane.
- Mô hình dữ liệu head/tail.
- Các quy trình nền tự động quản lý vòng đời dữ liệu.
...
# Output: 2022-12-20 15:10:16   20971520 bigfile
...
Lệnh `radosgw-admin object stat` có thể hiển thị manifest striping/parts của object qua metadata RGW:

```bash
radosgw-admin object stat --bucket BUCKET --object OBJECT | jq .
```

Tail objects trong ví dụ của chúng ta:

```bash
rados -p default.rgw.buckets.data ls | grep shadow_bigfile
# Output: 7fb0a3df-9553-4a76-938d-d23711e67677.34162.1__shadow_bigfile.2~E_PYNwiBq0la0EuZcCOY30KgmRrf1pV.1_1
#         7fb0a3df-9553-4a76-938d-d23711e67677.34162.1__shadow_bigfile.2~E_PYNwiBq0la0EuZcCOY30KgmRrf1pV.2_1
```

Tail objects thường giữ 4MB chunks của dữ liệu:
...
# Output: default.rgw.buckets.data/... mtime ... size 4194304
```
...
# Output: 7fb0a3df-9553-4a76-938d-d23711e67677.34162.1__shadow_5chuncks.2_r3yyxqL2hYs5DW32L9UXR3uawF4VEKL.2_1
